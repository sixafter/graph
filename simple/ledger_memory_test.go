...
func TestMemoryStorage(t *testing.T) {
	...
	t.Run("TestVertexOperations", func(t *testing.T) {
		t.Parallel()
		is := assert.New(t)
		...
		err = s.AddVertex(v.ID(), v.Value(), v.Properties())
		is.NoError(err)
		...
		err = s.ModifyVertex(1, v.Properties())
		is.NoError(err)
	...
	t.Run("TestEdgeOperations", func(t *testing.T) {
		t.Parallel()
		is := assert.New(t)
	...
	t.Run("TestWouldCreateCycle", func(t *testing.T) {
		t.Parallel()
		is := assert.New(t)
		...
		// Add edges
		e := NewEdgeWithOptions(1, 2)
		is.NoError(s.AddEdge(1, 2, e))
		e = NewEdgeWithOptions(2, 3)
